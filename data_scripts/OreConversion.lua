...
local function generateIcons(NewOreName, Ore)
    ...
    if (Ore.icon) then table.insert(retValue, oreIcon) end
...
local function CreateRecipies(fromItem, ore, oreResult)
    ...
    table.insert(
        ...
        {type = "unlock-recipe", recipe = newItemName}
    )
    table.insert(
        data.raw.technology[technologyName].effects,
        {
            type = "unlock-recipe",
            recipe = "ax-smelting-" .. newItemName
        }
    )
end
...
local function CreateNewMatterOreItem(NewOreName, Ore, oreResult)
    ...
    local result = {
        ...
        localised_name = {
            "",
            {"item-name.ax-matter-infused-ore"},
            " ",
...
local function CreateMatterOrePrototype(NewOreName, Ore, oreResult)
    ...
    newOre.localised_name = {
        "",
        {"entity-name.ax-matter-infused-ore"},
        " ",
        {"entity-name." .. Ore.name}
    }
    newOre.icon = nil
...
local function ProcessOres()
    ...
    for name, ore in pairs(util.table.deepcopy(data.raw["resource"])) do
        ...
        if (ore.minable.results) then
            for _, x in pairs(ore.minable.results) do if (x.type == "fluid") then skipOre = true end end
        end

        if (ore.minable.result) then
            if (ore.minable.result.type == "fluid") then skipOre = true end
        end
        -- end fluid check
