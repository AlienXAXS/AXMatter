    ...
    for k = #global.axmatter.oreConverters, 1, -1 do
        ...
        if (x.entity.valid and x.ore.valid) then
            ...
            if (percentage >= 100) then
                ...
                local oreArea = {
                    {
                        x.entity.position.x - BoxSize,
                        x.entity.position.y - BoxSize
                    },
                    {
                        x.entity.position.x + BoxSize,
                        x.entity.position.y + BoxSize
                ...
                for _, oreEnt in ipairs(x.entity.surface.find_entities_filtered{area = oreArea, name = x.ore.name}) do
                    local ent = oreEnt.surface.create_entity{
                        force = "neutral",
                        name = convertToOre.name,
                        position = oreEnt.position
                    }
...
function OreConverter.script_on_entity_destroyed(event)
    ...
    if (entity) then
        if entity.valid and entity.name == "ax-matter-ore-converter" then OreConverter.Cleanup(entity) end
    end
end
    ...
    if (entity) then
        if entity.valid and entity.name == "ax-matter-ore-converter" then
            ...
            if (#oreFinderResult == 0) then
                ...
                if (not OreConverter.getConvertToOre(oreFinderResult[1].name)) then
                    if not isRobot then
                        player.print({
                            "",
                            oreFinderResult[1].localised_name,
                            " is not a supported ore"
                        })
                        player.get_main_inventory().insert({name = entity.name, count = 1})
            ...
            local text = rendering.draw_text{
                ...
                surface = entity.surface,
                target = {
                    x = entity.position.x - BoxSize + 0.05,
                    y = entity.position.y - BoxSize
                }
            }

            table.insert(
                global.axmatter.oreConverters,
                {
                    entity = entity,
                    renderdata = {bb, text},
                    ore = oreFinderResult[1]
                }
            )
        end
    end
